#
# Script:	    pssService-refresh-pivot-reports.ps1
# Task:		    Refresh Excel Pivot charts and lists
# 
...
Write-Host "pobvol Service Solution: pssService-refresh-pivot-reports.ps1 started ...."
...
for ($i=1; $i -le 100; $i++)
{
    # $strMessage = "Reading row from pssService-Settings.xlsx: " + $i
    # Write-Host $strMessage
    # LogWrite $strMessage
...
If( $SPDomain -eq "null" -OR $SPDomain -eq "Enter your value" -OR
    ...
    $strMessage = "pobvol Service Solution: pssServiceGo.ps1"
    LogWrite $strMessage
    $strMessage = "    Processing service data skipped. Missing parameters!"
    Write-Host $strMessage
    LogWrite $strMessage
    $strMessage = "    Please check your entries in " +$MyPath +'\SystemSettings.xlsx'
    Write-Host $strMessage
    LogWrite $strMessage
}
...
If($strError -eq ""){
    try {
        # Excel-Datei "Dashboard.xlsm" aufrufen
        $fileName = $MyPath + '\reports\pssService-Dashboard.xlsm'
        $strMessage = "Refreshing Pivot charts and lists"
        Write-Host $strMessage
        LogWrite $strMessage
        $excel = New-Object -ComObject Excel.Application
        $excel.Visible = $false #Soll Excel angezeigt werden ($true) oder unsichtbar im Hintergrund laufen ($false)?
...
# ---------------------------------------------------------------------------
# Copy changed files from folder "reports" to SharePoint 
# ---------------------------------------------------------------------------

Write-Host "    " 
Write-Host "Copy changed file to your SharePoint document library"
$strSource = $MyPath + "\reports\*.*"
$strTarget = $SPPath + "\" + $SPReportsFolder
...
Write-Host "    " 
Write-Host "pobvol Service Solution: pssService-refresh-pivot-reports.ps1 finished."
